...
class WordPressClient:
    ...
    def get_content(self, kind: str, content_id: int) -> Optional[Content]:
        ...
        if not isinstance(item, dict) or "id" not in item:
            raise HttpError(0, self._url(f"{kind}/{content_id}"), "unexpected response shape for an item")
    ...
    def prepare_block_repair(
        ...
        working_raw: Optional[str] = None, raw_block_index: Optional[int] = None,
    ) -> str:
        ...
        raw_blocks = find_blocks(content.content_raw)
        raw_block = None
        if raw_block_index is not None and 0 <= raw_block_index < len(raw_blocks):
            # The caller mapped live blocks to post_content blocks one-to-one
            # (identical twins included); honour that mapping if it matches.
            cand = raw_blocks[raw_block_index]
            if match_block_in_raw(front_block, content.content_raw, exclude=set(range(len(raw_blocks))) - {cand.index}) is not None:
                raw_block = cand
        ...
        if target.raw_block >= 0 and raw_block.index != target.raw_block:
            raise OwnershipLost(
                f"the block resolves to post_content block {raw_block.index}; this repair "
                f"was keyed on block {target.raw_block}. Re-run 'audit --propose'."
            )
        ...
        if patch.get("op") == "rename_id":
            ...
            if content.front_html:
                live_blocks = find_blocks(content.front_html)
                live_refs = references_outside(live_blocks, old, front_block.index)
                # Map live blocks to post_content one-to-one, as the audit does;
                # a referencing block with no unclaimed match is not ours.
                claimed: set = set()
                owned_live: set = set()
                for lb in live_blocks:
                    if lb.plugin:
                        continue
                    m = match_block_in_raw(lb, content.content_raw, exclude=claimed)
                    if m is not None:
                        claimed.add(m.index)
                        owned_live.add(lb.index)
                foreign = [i for i in live_refs if i not in owned_live]
                if foreign:
                    raise PatchError(
